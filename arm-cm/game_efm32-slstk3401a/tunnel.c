...
// private:
    ...
    QAsm * mines[GAME_MINES_MAX];
    QAsm * mine1_pool[GAME_MINES_MAX];
    QAsm * mine2_pool[GAME_MINES_MAX];
    uint8_t blink_ctr;
    uint8_t last_mine_x;
    uint8_t last_mine_y;
...
//${App::AO_Tunnel} ..........................................................
QActive * const AO_Tunnel = Q_ACTIVE_UPCAST(&Tunnel_inst);
//$enddef${App::AO_Tunnel} ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//$define${App::QTicker_inst0} vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv

//${App::QTicker_inst0} ......................................................
QTicker QTicker_inst0;
//$enddef${App::QTicker_inst0} ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

// Active object definition ================================================
//$define${App::Tunnel_ctor_call} vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv

//${App::Tunnel_ctor_call} ...................................................
void Tunnel_ctor_call(void) {
    Tunnel_ctor(&Tunnel_inst);
}
//$enddef${App::Tunnel_ctor_call} ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    QEvt const * e)
{
    for (uint8_t n = 0U; n < GAME_MINES_MAX; ++n) {
        if (me->mines[n] != (QAsm *)0) { // is the mine used?
...
static void Tunnel_ctor(Tunnel * const me) {
    ...
    for (n = 0; n < GAME_MINES_MAX; ++n) {
        ...
        me->mines[n] = (QAsm *)0; // mine 'n' is unused
    }
...
static QState Tunnel_initial(Tunnel * const me, void const * const par) {
    ...
    QActive_subscribe(Q_ACTIVE_UPCAST(me), TIME_TICK_SIG);
    QActive_subscribe(Q_ACTIVE_UPCAST(me), PLAYER_TRIGGER_SIG);
    QActive_subscribe(Q_ACTIVE_UPCAST(me), PLAYER_QUIT_SIG);
    ...
    switch (e->sig) {
        ...
        case MINE_DISABLED_SIG: {
            Q_ASSERT((Q_EVT_CAST(MineEvt)->id < GAME_MINES_MAX)
                     && (me->mines[Q_EVT_CAST(MineEvt)->id] != (QAsm *)0));
            me->mines[Q_EVT_CAST(MineEvt)->id] = (QAsm *)0;
            status_ = Q_HANDLED();
            break;
        }
